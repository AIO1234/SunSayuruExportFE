  ...
  <div>
    <div>
      <b-form @submit.prevent>
        <validation-observer ref="GradingUpdateValidation">
          <b-row>
            <!-- grading -->
            ...
            <b-col md="12" class="mb-1 text-center">
              <b-button
                @click="validationGradingUpdateForm()"
                type="submit"
                variant="none"
...
export default {
  ...
  methods: {
    async validationGradingUpdateForm() {
      if (await this.$refs.GradingUpdateValidation.validate()) {
        await this.$vs.loading({
          scale: 0.8,
        });
        await gradingApi
          .updateGrading(this.form.id, this.form)
          .then(() => {
